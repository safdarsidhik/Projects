...
def predict():
    try:
        # ----- INPUTS EXACTLY LIKE DATASET -----
        exports_pct = float(request.form["exports_pct"])              # %
        imports_pct = float(request.form["imports_pct"])              # %
        population = float(request.form["population"])                # integer, NOT billions
        ...
        exchange_rate = float(request.form["exchange_rate"])          # LCU per USD

    except ValueError:
        return "Invalid input. Please enter valid numbers."

    # ---- Create feature vector in EXACT ORDER USED IN TRAINING ----
    features = np.array([[ 
        exports_pct,
        imports_pct,
        population,
    ...
    # ---- Convert back from log to BILLIONS USD ----
